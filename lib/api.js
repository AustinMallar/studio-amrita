...
const CATEGORY_SLUGS = {
  "Glow Bear Bundle": "glow-bear-bundle",
...
export function buildHomepageCollectionsQuery() {
  ...
  query HomepageCollections {
    glowBearBundle: productCategory(id: "${CATEGORY_SLUGS["Glow Bear Bundle"]}", idType: SLUG) {
      databaseId
      name
      description
      slug
...
function pickPrice(node) {
  ...
  const display = formatted ?? formatMoney(raw) ?? "—";
...
function mapCategoryBlock(cat) {
  ...
  return {
    ...
    slug: cat.slug,
    lifestyleImageUrl: cat.image?.sourceUrl ?? null,
    lifestyleImageAlt: cat.image?.altText ?? cat.name,
    products,
    minPriceFormatted:
      minPrice != null
...
function mapClassicVariableProduct(node) {
  ...
  const description = (node.description ?? "").replace(/<[^>]+>/g, "").trim();
...
/** Other products in the Glow Bear Bundle category (for PDP cross-links). */
function buildGlowBearBundleSiblingsQuery() {
  const catSlug = CATEGORY_SLUGS["Glow Bear Bundle"];
  return `
...
/**
 * @param {string} slug — WooCommerce product category slug (e.g. glow-bear-bundle)
...
export async function getProductCategoryCollection(slug) {
  ...
  if (!block && json?.errors?.length) {
    ...
    const needle =
      slug === CATEGORY_SLUGS["Glow Bear Bundle"]
        ? "Glow Bear Bundle"
...
export async function getHomepageCollections() {
  ...
  if (!bundle && !classic && !charms && errors?.length) {
    ...
    bundle = mapCategoryBlock(byName("Glow Bear Bundle"));
  ...
  return {
    ...
    rows: [
      {
        key: "bundle",
        categoryName: "Glow Bear Bundle",
        layoutType: "grid",
        shopHref: `/shop/${CATEGORY_SLUGS["Glow Bear Bundle"]}`,
...
export { CATEGORY_SLUGS, classicGlowBearSlug };
