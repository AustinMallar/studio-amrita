...
import { ProductCardHoverMedia } from "@/components/ProductCardHoverMedia";

export type HoverMediaKind = "video" | "gif" | "image";
...
export type UiProduct = {
  ...
  imageAlt: string;
  swatches: string[];
  /** First product gallery asset (GIF, short video, or still) — see WooCommerce product gallery. */
...
export function ProductCard({ product }: { product: UiProduct }) {
    ...
    <article className="flex flex-col gap-3 rounded-2xl bg-white/60 p-3 shadow-[0_2px_8px_rgba(92,77,77,0.07),0_1px_2px_rgba(92,77,77,0.04)] transition hover:shadow-[0_4px_14px_rgba(92,77,77,0.1)]">
      {hasHover ? (
        <ProductCardHoverMedia
          ...
          hoverMediaUrl={product.hoverMediaUrl!}
          hoverMediaAlt={product.hoverMediaAlt ?? ""}
          hoverMediaKind={product.hoverMediaKind!}
        />
      ) : (
  ...
  if (href) {
    return (
      <Link
        href={href}
        className="block outline-offset-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-dusty-rose"
      >
        {inner}
      </Link>
    );
